...
class TestUploadFileOrchestrator(unittest.TestCase):
    """Tests for _upload_file() orchestration and mode selection."""
    ...
    def test_default_mode_is_auto(self):
        """mode=None defaults to auto (small for small files)."""
        path = _make_temp_file()
        try:
            self.od._upload_file_small = MagicMock()
    ...
    def test_lowercases_attribute_name(self):
        ...
        try:
            self.od._upload_file("account", "guid-1", "new_Document", path, mode="small")
            # Third positional arg is the logical_name (lowercased)
            self.assertEqual(self.od._upload_file_small.call_args[0][2], "new_document")
        finally:
            os.unlink(path)
    ...
    def test_passes_mime_type_to_small(self):
        ...
        try:
            ...
            kwargs = self.od._upload_file_small.call_args[1]
    ...
    def test_passes_if_none_match_to_small(self):
        ...
        try:
            ...
            kwargs = self.od._upload_file_small.call_args[1]
            self.assertFalse(kwargs["if_none_match"])
        finally:
            os.unlink(path)
...
class TestUploadFileSmall(unittest.TestCase):
    ...
    def test_successful_upload(self):
        ...
        try:
            ...
            args, kwargs = self.od._request.call_args
            self.assertEqual(args[0], "patch")
            self.assertIn("new_document", args[1])
            self.assertEqual(kwargs["data"], b"PDF file content here")
        finally:
            os.unlink(path)
    ...
    def test_if_none_match_header(self):
        ...
        try:
            ...
            headers = self.od._request.call_args[1]["headers"]
...
class TestUploadFileChunk(unittest.TestCase):
    ...
    def _mock_init_response(self, location="https://example.com/session?token=abc", chunk_size=None):
    ...
    def test_init_patch_sends_chunked_header(self):
        ...
        try:
            ...
            self.assertEqual(init_call[1]["headers"]["x-ms-transfer-mode"], "chunked")
        finally:
            os.unlink(path)
    ...
    def test_init_url_contains_file_name(self):
        ...
        try:
            self.od._upload_file_chunk("accounts", "guid-1", "col", path)
            init_url = self.od._request.call_args_list[0][0][1]
    ...
    def test_content_range_headers(self):
        ...
        try:
            ...
            self.assertEqual(chunk_calls[0][1]["headers"]["Content-Range"], "bytes 0-9/25")
            self.assertEqual(chunk_calls[1][1]["headers"]["Content-Range"], "bytes 10-19/25")
            self.assertEqual(chunk_calls[2][1]["headers"]["Content-Range"], "bytes 20-24/25")
        finally:
            os.unlink(path)
    ...
    def test_chunk_content_length_header(self):
        ...
        try:
            ...
            self.assertEqual(chunk_calls[0][1]["headers"]["Content-Length"], "10")
            self.assertEqual(chunk_calls[1][1]["headers"]["Content-Length"], "5")
        finally:
            os.unlink(path)
    ...
    def test_chunk_sends_to_location_url(self):
        ...
        try:
            ...
            self.assertEqual(chunk_call[0][1], session_url)
        finally:
            os.unlink(path)
    ...
    def test_if_none_match_on_init(self):
        ...
        try:
            ...
            init_headers = self.od._request.call_args_list[0][1]["headers"]
    ...
    def test_chunk_requests_accept_206_and_204(self):
        ...
        try:
            ...
            for chunk_call in self.od._request.call_args_list[1:]:
                self.assertEqual(chunk_call[1]["expected"], (206, 204))
        finally:
            os.unlink(path)
