...
class TestAsyncBatchOperationsNamespace:
    """Tests that AsyncBatchOperations and AsyncBatchRequest are correctly wired on the client."""

    def test_namespace_exists(self):
        """client.batch exposes an AsyncBatchOperations instance."""
        credential = AsyncMock(spec=AsyncTokenCredential)
        client = AsyncDataverseClient("https://example.crm.dynamics.com", credential)
        assert isinstance(client.batch, AsyncBatchOperations)

    def test_new_returns_batch_request(self):
        """client.batch.new() returns a fresh AsyncBatchRequest."""
        credential = AsyncMock(spec=AsyncTokenCredential)
        client = AsyncDataverseClient("https://example.crm.dynamics.com", credential)
...
class TestAsyncBatchRecordOperations:
    """Tests for local (no-HTTP) assembly of record operations onto an AsyncBatchRequest."""
    ...
    def test_create_single_appends_record_create(self):
        """A single-dict create call appends one _RecordCreate with the correct table and data."""
    ...
    def test_create_list_appends_record_create(self):
        """A list-of-dicts create call appends one _RecordCreate whose data is the whole list."""
        batch = self._make_batch()
    ...
    def test_update_appends_record_update(self):
        """An update call appends one _RecordUpdate with the correct table and id."""
        batch = self._make_batch()
    ...
    def test_delete_appends_record_delete(self):
        """A delete call appends one _RecordDelete with the correct table and id."""
        batch = self._make_batch()
    ...
    def test_get_appends_record_get(self):
        """A get call appends one _RecordGet with the correct table, id, and select list."""
        batch = self._make_batch()
    ...
    def test_upsert_appends_record_upsert(self):
        """An upsert call with UpsertItem objects appends one _RecordUpsert for the table."""
        batch = self._make_batch()
    ...
    def test_upsert_empty_list_raises(self):
        """Passing an empty list to upsert raises TypeError."""
        batch = self._make_batch()
        with pytest.raises(TypeError):
            batch.records.upsert("account", [])

    def test_multiple_operations_append_in_order(self):
        """Multiple operations are queued in the order they were added."""
        batch = self._make_batch()
...
class TestAsyncChangeSet:
    """Tests for changeset construction, context-manager usage, and per-operation isolation."""
    ...
    def test_changeset_appended_to_items(self):
        """Calling changeset() appends a _ChangeSet to the batch item list."""
        batch = self._make_batch()
        batch.changeset()
        assert len(batch._items) == 1
    ...
    def test_changeset_is_context_manager(self):
        """AsyncChangeSet can be used as a context manager and exposes a records namespace."""
        batch = self._make_batch()
    ...
    def test_changeset_records_create_adds_to_changeset(self):
        """A create inside a changeset block adds one operation to that changeset."""
        batch = self._make_batch()
        with batch.changeset() as cs:
    ...
    def test_changeset_records_update_adds_to_changeset(self):
        """An update inside a changeset block adds one operation to that changeset."""
        batch = self._make_batch()
        with batch.changeset() as cs:
    ...
    def test_changeset_records_delete_adds_to_changeset(self):
        """A delete inside a changeset block adds one operation to that changeset."""
        batch = self._make_batch()
        with batch.changeset() as cs:
    ...
    def test_multiple_changesets_are_separate(self):
        """Two consecutive changesets are stored as independent items with their own operations."""
        batch = self._make_batch()
...
class TestAsyncBatchQueryOperations:
    """Tests for validation in AsyncBatchQueryOperations.sql."""
    ...
    def test_sql_empty_raises(self):
        """An empty SQL string raises ValidationError."""
    ...
    def test_sql_non_string_raises(self):
        """A non-string SQL argument raises ValidationError or TypeError."""
        from PowerPlatform.Dataverse.core.errors import ValidationError
...
class TestAsyncBatchRequestExecute:
    """Tests that AsyncBatchRequest.execute delegates to _AsyncBatchClient correctly."""

    async def test_execute_calls_batch_client_execute(self):
        """execute() constructs _AsyncBatchClient with the OData client and calls execute on it."""
    ...
    async def test_execute_passes_continue_on_error(self):
        """execute(continue_on_error=True) forwards that flag to the batch client."""
        client, od = _make_client_with_mock_odata()
        batch = client.batch.new()
...
class TestAsyncBatchRecordUpsertDict:
    """Tests for the dict-form normalisation path in AsyncBatchRecordOperations.upsert."""
    ...
    def test_upsert_dict_form_normalised(self):
        """A dict with alternate_key/record keys is normalised into a _RecordUpsert item."""
        batch = self._make_batch()
    ...
    def test_upsert_invalid_dict_raises(self):
        """A dict missing the expected keys raises TypeError."""
        batch = self._make_batch()
        with pytest.raises(TypeError):
...
class TestAsyncBatchQuerySqlSuccess:
    """Tests for the success path of AsyncBatchQueryOperations.sql."""
    ...
    def test_sql_success_appends_query_sql(self):
        """A valid SQL string appends a _QuerySql item with the correct sql attribute."""
        batch = self._make_batch()
...
class TestAsyncBatchTableOperations:
    """Tests for local (no-HTTP) assembly of all table operations onto an AsyncBatchRequest."""
    ...
    def test_create_appends_table_create(self):
        """tables.create() appends a _TableCreate item with the correct table name."""
        batch = self._make_batch()
    ...
    def test_delete_appends_table_delete(self):
        """tables.delete() appends a _TableDelete item with the correct table name."""
        batch = self._make_batch()
    ...
    def test_get_appends_table_get(self):
        """tables.get() appends a _TableGet item with the correct table name."""
        batch = self._make_batch()
    ...
    def test_list_passes_filter_and_select(self):
        """tables.list() stores filter and select on the appended item."""
        batch = self._make_batch()
    ...
    def test_add_columns_appends_table_add_columns(self):
        """tables.add_columns() appends a _TableAddColumns item for the named table."""
        batch = self._make_batch()
    ...
    def test_delete_relationship_appends(self):
        """tables.delete_relationship() appends a _TableDeleteRelationship with the correct id."""
        batch = self._make_batch()
    ...
    def test_get_relationship_appends(self):
        """tables.get_relationship() appends a _TableGetRelationship with the correct schema name."""
        batch = self._make_batch()
    ...
    def test_create_lookup_field_appends(self):
        """tables.create_lookup_field() appends a _TableCreateLookupField with referencing/referenced tables."""
        batch = self._make_batch()
    ...
    def test_multiple_table_ops_in_order(self):
        """Multiple table operations are queued in the order they were added."""
        batch = self._make_batch()
...
class TestAsyncBatchDataFrameOperations:
    """Tests for AsyncBatchDataFrameOperations: DataFrame/Series-to-batch-item conversion."""
    ...
    def test_create_from_dataframe(self):
        """A valid DataFrame is converted to a list-of-dicts _RecordCreate item."""
        import pandas as pd
    ...
    def test_create_non_dataframe_raises(self):
        """Passing a non-DataFrame to dataframe.create raises TypeError."""
        batch = self._make_batch()
        with pytest.raises(TypeError):
            batch.dataframe.create("account", [{"name": "X"}])

    def test_create_empty_dataframe_raises(self):
        """Passing an empty DataFrame to dataframe.create raises ValueError."""
        import pandas as pd
    ...
    def test_update_from_dataframe(self):
        """A valid DataFrame produces a _RecordUpdate item in the batch."""
        import pandas as pd
    ...
    def test_update_non_dataframe_raises(self):
        """Passing a non-DataFrame to dataframe.update raises TypeError."""
        batch = self._make_batch()
        with pytest.raises(TypeError):
            batch.dataframe.update("account", "bad", "accountid")

    def test_update_empty_dataframe_raises(self):
        """Passing an empty DataFrame to dataframe.update raises ValueError."""
        import pandas as pd
    ...
    def test_update_missing_id_column_raises(self):
        """A DataFrame without the specified id_column raises ValueError."""
        import pandas as pd
    ...
    def test_delete_from_series(self):
        """A valid Series of IDs produces a _RecordDelete item in the batch."""
        import pandas as pd
    ...
    def test_delete_non_series_raises(self):
        """Passing a non-Series to dataframe.delete raises TypeError."""
        batch = self._make_batch()
        with pytest.raises(TypeError):
            batch.dataframe.delete("account", ["guid-1"])

    def test_delete_empty_series_is_noop(self):
        """An empty Series passed to dataframe.delete results in no items being queued."""
        import pandas as pd
