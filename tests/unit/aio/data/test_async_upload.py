...
class TestUploadFileSmall:
    ...
    async def test_default_content_type_is_octet_stream(self):
        """Omitting content_type sends application/octet-stream in the Content-Type header."""
        client = _MockUploadClient()
        client._request.return_value = _mock_response(status_code=204)
    ...
    async def test_custom_content_type_used(self):
        """A caller-supplied content_type value is forwarded as the Content-Type header."""
        client = _MockUploadClient()
        client._request.return_value = _mock_response(status_code=204)
        path = _tmp_file(b"data")
        try:
    ...
    async def test_if_none_match_true_sets_if_none_match_header(self):
        """if_none_match=True sets If-None-Match: null and omits If-Match."""
        client = _MockUploadClient()
        client._request.return_value = _mock_response(status_code=204)
        path = _tmp_file(b"data")
        try:
    ...
    async def test_if_none_match_false_sets_if_match_star(self):
        """if_none_match=False sets If-Match: * and omits If-None-Match."""
        client = _MockUploadClient()
        client._request.return_value = _mock_response(status_code=204)
        path = _tmp_file(b"data")
        try:
    ...
    async def test_x_ms_file_name_header_set(self):
        """Sets the x-ms-file-name header to the basename of the upload path."""
        client = _MockUploadClient()
        client._request.return_value = _mock_response(status_code=204)
        path = _tmp_file(b"data")
        try:
    ...
    async def test_uses_patch_method(self):
        """Issues the upload as an HTTP PATCH request."""
        client = _MockUploadClient()
        client._request.return_value = _mock_response(status_code=204)
        path = _tmp_file(b"data")
        try:
...
class TestUploadFileChunk:
    """Tests for _upload_file_chunk multi-part chunked upload."""

    async def test_uploads_successfully_in_chunks(self):
        """Completes a chunked upload by issuing at least an init and one chunk request."""
        client = _MockUploadClient()
    ...
    async def test_missing_location_header_raises(self):
        """Raises RuntimeError when the init response omits the Location header."""
        client = _MockUploadClient()
        path = _tmp_file(b"data")
        try:
    ...
    async def test_if_none_match_true_sets_header_on_init(self):
        """if_none_match=True sets If-None-Match: null on the init request and omits If-Match."""
        client = _MockUploadClient()
        path = _tmp_file(b"data")
        try:
    ...
    async def test_if_none_match_false_sets_if_match_star_on_init(self):
        """if_none_match=False sets If-Match: * on the init request and omits If-None-Match."""
        client = _MockUploadClient()
        path = _tmp_file(b"data")
        try:
            init_resp = _mock_response(headers={"Location": "https://example.com/session/tok"})
