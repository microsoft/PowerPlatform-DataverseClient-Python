...
from typing import TYPE_CHECKING, Any, AsyncGenerator, Dict, List, Optional, Union, overload
...
class AsyncRecordOperations:
    ...
    :param client: The parent :class:`~PowerPlatform.Dataverse.aio.AsyncDataverseClient` instance.
    ...
    async def get(
        ...
        *,
        ...
        :rtype: AsyncGenerator[list[~PowerPlatform.Dataverse.models.record.Record], None]
    ...
    async def get(
        ...
        **Fetch multiple records** -- ``get(table, *, select=..., filter=..., ...)``
        ...
        Omit ``record_id`` to perform a paginated fetch. Returns an async
        generator that yields one page (list of Records) at a time.
        ...
        :return: A single :class:`~PowerPlatform.Dataverse.models.record.Record`
            when ``record_id`` is provided, or an async generator yielding pages
            (lists of Records) when fetching multiple records.
    ...
    async def upsert(
        ...
        When ``items`` contains a single entry, performs a single upsert via
        PATCH using the alternate key in the URL. When ``items`` contains
        multiple entries, uses the ``UpsertMultiple`` bulk action.

        Each item must be either a :class:`~PowerPlatform.Dataverse.models.upsert.UpsertItem`
        or a plain ``dict`` with ``"alternate_key"`` and ``"record"`` keys
        (both dicts).
