...
class AsyncFileOperations:
    ...
    Accessed via ``client.files``.  Async counterpart of
    ...
    async def upload(
        ...
        *,
        ...
        :param file_column: Schema name of the file column attribute.
        :type file_column: :class:`str`
        :param path: Local filesystem path to the file.
        :type path: :class:`str`
        :param mode: Upload strategy: ``"auto"`` (default), ``"small"``,
            or ``"chunk"``.
        ...
        :param if_none_match: When True (default), sends
            ``If-None-Match: null`` to only succeed if the column is empty.
